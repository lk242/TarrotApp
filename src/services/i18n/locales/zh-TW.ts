const zhTW = {
  ...
  // 服務說明（首頁 + 購買頁共用）
  serviceInfo: {
    homeTitle: 'AI 塔羅線上解讀服務',
    homeDesc: '神秘塔羅提供線上 AI 塔羅文字解讀服務。使用者可選擇牌陣、輸入問題、抽取塔羅牌，並取得由 AI 依照牌義、牌位與問題脈絡生成的個人化解讀內容。',
    homeCreditsNote: '付款方案購買的是本站 AI 塔羅解讀服務額度，可用於全新占卜與同一輪占卜後的延伸追問；服務額度僅限本站帳戶本人於本站使用，不具貨幣價值，不得轉讓、兌換現金或作為其他商品交易使用。',
    billingTitle: 'AI 塔羅解讀服務說明',
    billingDesc: '本頁銷售內容為「神秘塔羅」線上 AI 塔羅解讀服務額度，可用於啟動全新牌陣解讀與同一輪解讀後的延伸追問服務。服務額度僅限本站帳戶本人於本站使用，不具貨幣價值，不得轉讓、兌換現金或用於其他平台商品交易。',
    billingHow: '服務提供方式：付款完成後，系統會自動將可使用的解讀服務額度加入會員帳戶，使用者可在本站輸入問題、抽牌並取得 AI 產生的塔羅文字解讀。',
