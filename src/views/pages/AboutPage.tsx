  ...
  return (
    <div className="flex flex-1 flex-col items-center px-6 py-20">
      ...
      <motion.div {...fadeUp()} className="mb-16 max-w-2xl text-center">
        ...
        <h1 className="mb-4 text-3xl font-bold tracking-wider text-[var(--color-accent-gold)] md:text-4xl">
          關於神秘塔羅
        </h1>
      ...
      <motion.div {...fadeUp(0.15)} className="mb-16 w-full max-w-4xl">
        <h2 className="mb-8 text-center text-xl font-bold text-[var(--color-accent-gold)]">
          ✦ 核心特色 ✦
        </h2>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f, i) => (
            <motion.div
              key={f.title}
              {...fadeUp(0.2 + i * 0.08)}
              className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-6 transition-all hover:border-[var(--color-border-hover)] hover:shadow-[var(--shadow-card-hover)]"
            >
              <div className="mb-3 text-3xl">{f.icon}</div>
              <h3 className="mb-2 text-sm font-bold text-[var(--color-accent-gold-light)]">
                {f.title}
              </h3>
      ...
      <motion.div {...fadeUp(0.3)} className="mb-16 w-full max-w-3xl">
        ...
        <div className="space-y-4">
          {SPREADS_INFO.map((s) => (
            <div
              key={s.name}
              className="flex items-start gap-5 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 transition-all hover:border-[var(--color-border-hover)]"
            >
              <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-xl bg-[var(--color-accent-gold)]/10 text-2xl text-[var(--color-accent-gold)]">
                {s.icon}
              </div>
              <div className="flex-1">
                <div className="mb-1 flex items-center gap-3">
                  <h3 className="text-base font-bold text-[var(--color-text-primary)]">{s.name}</h3>
                  <span className="rounded-full bg-[var(--color-accent-purple)]/15 px-2.5 py-0.5 text-[11px] text-[var(--color-accent-purple-light)]">
                    {s.cards}
                  </span>
      ...
      <motion.div {...fadeUp(0.4)} className="mb-16 w-full max-w-2xl">
        ...
        <div className="space-y-3">
          {TAROT_TIPS.map((tip, i) => (
            ...
            >
              <span className="mt-0.5 text-[var(--color-accent-gold)]">✦</span>
      ...
      <motion.div {...fadeUp(0.5)} className="mb-16 w-full max-w-2xl">
        <h2 className="mb-6 text-center text-xl font-bold text-[var(--color-accent-gold)]">
          ✦ 韋特塔羅的歷史 ✦
        </h2>
      ...
      <motion.div {...fadeUp(0.6)} className="w-full max-w-xl text-center">
        <div className="rounded-xl border border-[var(--color-border)]/50 bg-[var(--color-bg-secondary)] p-6">
          <p className="text-xs leading-relaxed text-[var(--color-text-muted)]">
            ⚠️ 免責聲明：塔羅牌是一種自我探索與反思的工具，並非科學預測方法。
